...
T.test("snapshot request protocol round trips requester and exactly one target", function()
    ...
    local payload = assert(GGM.EncodeSyncSnapshotRequest(requester, target))
...
T.test("complete snapshot response round trips every tracked slot and sequence", function()
    ...
    local payload = assert(GGM.EncodeSyncSnapshotResponse(target, makeSnapshot(GGM), 12))
    local message, err = GGM.DecodeSyncMessage(payload)
    T.assertNil(err)
...
T.test("protocol rejects identity keys that do not match name and realm", function()
    ...
    local payload, err = GGM.EncodeSyncSnapshotRequest(identity, makeIdentity("Bob", "Silvermoon", nil))
    T.assertNil(payload)
...
T.test("protocol rejects malformed length prefixes and trailing data", function()
    ...
    local valid = assert(GGM.EncodeSyncSnapshotRequest(makeIdentity("Bob", "Silvermoon", "Player-1234-BBBB"), makeIdentity("Alice", "Silvermoon", nil)))
