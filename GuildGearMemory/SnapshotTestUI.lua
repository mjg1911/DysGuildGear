...
local function missingModel()
    return {
        hasSnapshot = false,
        emptyStateText = "No saved snapshot",
        slots = {},
    }
end
...
local function hasValidDisplayShape(record)
    ...
    if type(record.gear) ~= "table" or record.gear.complete ~= true then
        return false
    end

    if type(record.gear.capturedAt) ~= "number" then
        return false
    end

    if type(record.gear.slots) ~= "table" then
        return false
    end
...
function GGM.BuildSnapshotViewModel(record, formatTime)
    if not hasValidDisplayShape(record) then
        return missingModel()
    end
    ...
    if type(formatTime) == "function" then
        capturedAtText = formatTime("%Y-%m-%d %H:%M:%S", record.gear.capturedAt)
    end
...
function GGM.RenderSnapshotViewModel(frame, model)
    if not model.hasSnapshot then
        frame.emptyState:SetText(model.emptyStateText)
        frame.emptyState:Show()
        setMetadataVisible(frame, false)

        for _, row in ipairs(frame.slotRows) do
            row:Hide()
        end
    ...
    for index, slot in ipairs(model.slots) do
        local row = frame.slotRows[index]
        row:SetText(slot.key .. ": " .. slot.valueText)
        row:Show()
    end

    for index = #model.slots + 1, #frame.slotRows do
        frame.slotRows[index]:Hide()
    end
end
